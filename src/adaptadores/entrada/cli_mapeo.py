...
def _parse(argv: list[str] | None) -> argparse.Namespace:
    ...
    p.add_argument(
        "--umbral-directo", type=float, default=None, help="Pisa mapear_historias.umbral_directo."
    )
    p.add_argument(
        "--umbral-tentativo",
        type=float,
        default=None,
        help="Pisa mapear_historias.umbral_tentativo.",
    )
    p.add_argument(
        "--concurrencia", type=int, default=None, help="Pisa mapear_historias.concurrencia."
    )
    p.add_argument(
        "--sin-operaciones",
        action="store_true",
...
def _resumen(resultado) -> None:
    _ROL = {
        "OWNED_CONTRACT": "OWNED",
        "CONSUMED_DEPENDENCY": "CONSUMED",
        "RELATED_NOT_OWNED": "RELATED",
    }
    print("\n" + "=" * 72)
    ...
    for h in resultado.historias:
        ...
        print(
            f"    directos={h.total_directos}  tentativos={h.total_tentativos}  descartados={h.total_descartados}"
        )
        for etq, grupo in (
            ("D", h.service_domains.candidatos_directos),
            ("T", h.service_domains.candidatos_tentativos),
        ):
            for a in grupo:
                area = f"  · {a.business_area}" if a.business_area else ""
                print(
                    f"    [{etq} {a.confianza_pct:3d}%] {a.service_domain}  "
                ...
                for bq in a.bq_personalizados_propuestos:
                    print(
                        f"            op personalizada (NO oficial, revisar): {bq.operation_id}  "
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        caso_uso = crear_caso_uso_mapeo(
            config, proveedor=args.proveedor, actualizar_cache_bian=args.actualizar_cache_bian
        )
