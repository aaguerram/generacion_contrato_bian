...
> Estado y pendientes de generacion_contrato_bian al 2026-09-14 (todo en origin/main, commits 175669c, 4112f93 y c4bbda8; 257 tests en verde, sin red y sin LLM).
>
> HECHO: los 7 puntos de la lista anterior estan cerrados. (1) La ponderacion de la fusion RRF ya no es el problema: la causa real del canal disperso debil era el IDIOMA, no los pesos -ver la memoria de causa raiz-; fusion_rrf acepta k y pesos, y se añadio BM25 con puente ES->EN. (2) El corpus dorado pasa a 97 casos CON CAPAS declaradas, pero la capa de negocio sigue teniendo 6: las otras 91 son consultas-nombre generadas, regresion objetiva de validar-sd, no poder estadistico. (3) Sigue pendiente el canary, ver abajo. (4)-(5) siguen pendientes, ver abajo. (6) CatalogoBomPuml ya usa bom_diagram.puml_path del landscape. (7) Las 2 discrepancias de documentation se resolvieron con un invariante estructural (la seccion '** 1. Role **' debe coincidir con role_definition; 338/338).
>
> Ademas se implementaron los 6 puntos del plan de precision, todos OFF por defecto: cache de nodos en disco + defer + durabilidad, BM25 + RRF ponderado, CAG escalonado, corpus por capas + generador + skill corpus-dorado-bian, el grafo como confirmacion determinista del conflicto de ownership, y CRAG de una vuelta.
>
> PENDIENTE, por orden: (1) CANARY CON LLM REAL (scripts/evaluate_retrieval/canary.py --proveedor ollama), flag por flag, mirando ownership_conflict_rate y operation_grounding_rate y no solo recall. Ahora es barato porque la cache de nodos solo repaga lo que cambio. NINGUN flag se enciende sin esto. (2) AMPLIAR LA CAPA hu_real del corpus (6 consultas): sigue siendo el cuello y no se fabrica; protocolo en la skill. (3) MEDIR CAG CONTRA RETRIEVAL en hu_real con LLM real: el coste en tokens esta medido (~27k -> ~48k -> ~54k), el efecto en la decision no. (4) Encadenar scripts/enrich_service_landscape/ al final de scripts/generate_matrix_view/, que hoy pisaria el enriquecimiento al regenerar. (5) ownership_conflict_rate sigue metiendo en el denominador hallazgos sobre SD ya descartados; la señal de grafo da una tasa paralela que descuenta el ruido pero no arregla el denominador. (6) Entrenar un modelo propio sigue descartado hasta tener el corpus del punto 2. (7) pgvector no se implementa mientras la ADR no cambie. (8) Checkpointer persistente: durabilidad sync usa InMemorySaver, asi que no sobrevive a la muerte del proceso; hoy eso lo cubre la cache de nodos, y un resume real exigiria langgraph-checkpoint-sqlite (dependencia nueva). (9) El harness E2E no acepta --config ni --proveedor: ejecutar_caso() llama a cargar_settings() sin argumentos, asi que correr E2E con los flags nuevos o contra el modelo local exige editar config.yaml. Falta un override por variable de entorno.
>
> Entorno: exportar HF_HOME=~/.cache/hf-local (la cache global de HuggingFace es de root); el cross-encoder ya esta descargado ahi (2,2 GB). Qdrant quedo levantado con su volumen poblado (cd infra/retrieval && docker compose --profile qdrant down para pararlo conservando datos). Ollama local sirve qwen3.8:27b-q8_0 (chat) y qwen3-embedding:8b. git push necesita credential.helper='!gh auth git-credential' (no hay helper configurado ni GITHUB_TOKEN).
