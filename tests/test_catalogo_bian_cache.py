...
class TestCatalogoBianCache(unittest.TestCase):
    def test_cache_existente_no_toca_red(self):
        with tempfile.TemporaryDirectory() as tmp:
            root=Path(tmp); legacy=root/"ops.json"; legacy.write_text('{"service_domains":{}}',encoding="utf-8")
            cache=root/"cache"/"release14.0.0"; cache.mkdir(parents=True)
            (cache/"PartyAuthentication.json").write_text(json.dumps({"service_domain":"Party Authentication","release":"14.0.0","operations":[],"evidence":{"content_sha256":"abc"}}),encoding="utf-8")
    ...
    def test_descarga_ausente_y_refresh_explicito(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("urllib.request.urlopen",side_effect=calls): ev=cat.asegurar(["Party Authentication"])["Party Authentication"]
            self.assertEqual(ev.estado,"VERIFIED"); self.assertEqual(cat.operaciones_de("Party Authentication")[0].operation_id,"Evaluate")
            ...
            with patch("urllib.request.urlopen",return_value=_Response(oas)): refreshed=cat.asegurar(["Party Authentication"],actualizar=True)
    ...
    def test_sin_red_ni_cache_es_evidencia_no_disponible(self):
        with tempfile.TemporaryDirectory() as tmp:
            root=Path(tmp); legacy=root/"ops.json"; legacy.write_text('{"service_domains":{}}',encoding="utf-8")
            cat=CatalogoBianCache(legacy,root/"cache")
            with patch("time.sleep"), patch("urllib.request.urlopen",side_effect=urllib.error.URLError("offline")):
                ev=cat.asegurar(["Party Authentication"])["Party Authentication"]
            self.assertEqual(ev.estado,"BIAN_EVIDENCE_UNAVAILABLE")

    _OAS_V2 = (b"openapi: 3.0.1\n"
               b"paths:\n"
    ...
    def test_v2_extrae_schemas_con_cuerpo_request_response_y_catalogo(self):
        with tempfile.TemporaryDirectory() as tmp:
            root=Path(tmp); legacy=root/"ops.json"; legacy.write_text('{"service_domains":{}}',encoding="utf-8")
            cat=CatalogoBianCache(legacy,root/"cache")
            with patch("urllib.request.urlopen",side_effect=[_Response(b'{"sha":"c0ffee"}'),_Response(self._OAS_V2)]):
            ...
            self.assertEqual(ops["EvaluatePassword"].parent_control_record,"PartyAuthenticationAssessment")
            ...
            self.assertEqual([c["name"] for c in catlg["control_records"]],["PartyAuthenticationAssessment"])
            self.assertEqual(catlg["behavior_qualifiers"][0]["parent_control_record"],"PartyAuthenticationAssessment")
