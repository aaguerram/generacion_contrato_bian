...
class TestCalcularScore(unittest.TestCase):
    ...
    def test_matchea_grupo_cr_camelcase(self):
        op = OperacionBian(
            ...
            path="/Evaluate",
            tipo="CR",
            grupo="TransactionAuthorizationAssessment",
            summary="",
            description="",
        )
        d, _ = calcular_score(
            _p(accion_objeto="authorization transaction"), self.ENTRADA, [op], self.EV
        )
    ...
    def test_coherencia_jerarquia_usa_area_y_dominio(self):
        ...
        self.assertGreater(
            d.coherencia_jerarquia, 0.0
        )  # 'transaction'/'processing' están en area/domain
    ...
    def test_rubrica_estructurada_sustituye_al_lexico(self):
        ...
        d, _ = calcular_score(
            _p(accion_objeto="", match_service_role=3, match_objeto_negocio=3),
            self.ENTRADA,
            [],
            self.EV,
        )
    ...
    def test_confianza_libre_del_llm_no_entra_al_score(self):
        alta = calcular_score(_p(confianza=1.0, match_service_role=1), self.ENTRADA, [], self.EV)[
            0
        ].total
        baja = calcular_score(_p(confianza=0.0, match_service_role=1), self.ENTRADA, [], self.EV)[
            0
        ].total
    ...
    def test_evidencia_ausente_penaliza(self):
        ...
        sin = calcular_score(
            _p(match_service_role=3),
            self.ENTRADA,
            [],
            EvidenciaBian(estado="BIAN_EVIDENCE_UNAVAILABLE"),
        )[0].total
        self.assertGreater(con, sin)

    def test_match_action_sube_la_accion_oficial(self):
        d, _ = calcular_score(
            _p(accion_objeto="", match_service_role=0, match_action=3), self.ENTRADA, [], self.EV
        )
        self.assertEqual(d.accion_oficial, 1.0)

    def test_ambiguedad_alta_penaliza(self):
        alto = calcular_score(
            _p(match_service_role=3, ambiguity="NONE"), self.ENTRADA, [], self.EV
        )[0].total
        bajo = calcular_score(
            _p(match_service_role=3, ambiguity="HIGH"), self.ENTRADA, [], self.EV
        )[0].total
        self.assertGreater(alto, bajo)

    def test_evidence_quality_debil_penaliza_frente_a_no_puntuada(self):
        neutral = calcular_score(
            _p(match_service_role=3, evidence_quality=0), self.ENTRADA, [], self.EV
        )[0].total
        debil = calcular_score(
            _p(match_service_role=3, evidence_quality=1), self.ENTRADA, [], self.EV
        )[0].total
